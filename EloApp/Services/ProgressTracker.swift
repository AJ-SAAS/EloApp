...
final class ProgressTracker {
    ...
    func trackWordSpoken() {
        ...
        updateDayTracking() // Any practice counts toward streak/day
    }
    ...
    private func updateStreak(on today: Date, lastDate: Date?) {
        ...
        } else if lastDate == nil || !Calendar.current.isDate(lastDate!, inSameDayAs: today) {
            // First day ever, or break in streak → start at 1
    ...
    func resetWeekIfNeeded() {
        ...
        if todayWeekday == 1 { // Sunday — start of new week
            for day in 1...7 {
